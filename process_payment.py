...
# Temporary: Initialize plates_log.csv with correct headers and provided entry
# Remove this block after running once to create the CSV
with open('plates_log.csv', 'w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(['Plate Number', 'Payment Status', 'Timestamp', 'Payment Timestamp'])
    writer.writerow(['RAH972U', '0', '2025-05-07 17:26:34', ''])

# Configure the serial port (adjust '/dev/cu.usbmodem101' to your Arduino's port)
...
def read_last_unpaid_entry(plate):
    ...
    try:
        with open('plates_log.csv', 'r') as file:
            ...
            # Validate headers
            expected_headers = ['Plate Number', 'Payment Status', 'Timestamp', 'Payment Timestamp']
            if reader.fieldnames != expected_headers:
                print_boxed_message("Error: Invalid CSV Headers", "!")
                print(f"[{get_timestamp()}] Expected headers: {expected_headers}, Got: {reader.fieldnames}\n")
                return None
            entries = []
            for row in reader:
                # Strip whitespace from plate and status
                if row['Plate Number'].strip() == plate and row['Payment Status'].strip() == '0':
                    entries.append(row)
            if not entries:
                print_boxed_message("Warning: No Unpaid Entry Found", "!")
                print(f"[{get_timestamp()}] No unpaid entry for plate {plate}.\n")
                return None
            return entries[-1]
